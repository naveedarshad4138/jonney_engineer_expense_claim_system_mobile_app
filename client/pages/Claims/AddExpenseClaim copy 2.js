  ...
  const onSubmit = () => {
    for (let c = 0; c < expenses.length; c++) {
      for (let j = 0; j < jobs.length; j++) {
        ...
        if (jobData.noReceiptFlag && jobData.noReceiptReason.trim() === '') {
          Alert.alert(
            'Validation Error',
...
postData('form/expense-claims', transformedData);
  ...
  // Calculate the dynamic width for the entire table based on jobs count
  const tableWidth = 150 + jobs.length * 140 + 90;
  return (
