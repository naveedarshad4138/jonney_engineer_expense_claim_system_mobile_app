...
import { AllStackAIDeepSearchResults } from '../DeepSearch/AllStackAIDeepSearchResults';
...
export const ApiHistoryResultsModal = ({ isOpen, setIsModalOpen, resultss }) => {
  const OcrolusbookData = resultss?.summary?.data; // adjust key if it's named differently
  console.log(resultss?.websiteSearch?.data?.outputs)
  return (
      ...
      <div className="modal-content">
        ...
        <div className="modal-body" id="api-results">
          <div className="section">
            ...
            {resultss?.courtRecords?.length > 0 ? (
              resultss.courtRecords[0].raw_response.caseSearchResultArray.map((caseItem, index) => (
          ...
          {/* ///////////////////// Deep search adn website analysis /////////////////////// */}
          <div className="ai-results-container">
                <div className="ai-section section">
                  <h3>🧠 Deep Business + Owner Insight</h3>
                  {resultss?.deepsearch?.data?.outputs['out-0'] ? (
                    <ReactMarkdown>{resultss?.deepsearch?.data?.outputs['out-0']}</ReactMarkdown>
                  ):"No records found"}
                </div>
                ...
                <div className="ai-section section">
                  ...
                  {resultss?.websiteSearch?.data?.outputs['out-0'] ? (() => {
                      // Clean and parse the string from out-0
                      let parsed = null;
                      try {
                        const cleaned = resultss.websiteSearch.data.outputs['out-0']
                          .replace(/'/g, '"')
                      ...
                      return parsed ? (
                        <>
                          ...
                          {parsed?.events?.registration && (
                            <p><strong>Registered Date:</strong> {formatDate(parsed.events.registration)}</p>
                          )}
    </>
  ) : (
    <p className="error-text">Invalid website data format</p>
  );
                  })():"No records found"
}
          ...
          <div className="section">
            ...
            <DataMerchResults data={resultss?.datamerch} />
            </div>
